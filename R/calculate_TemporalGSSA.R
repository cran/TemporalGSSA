...
#' biochemical network will generate a dataset of randomly chosen timesteps along with changes to the
#' corresponding number of molecules. This constitutes a set observations for a specific simulation time.
#'
#' The temporal profile of a metabolite is a pre-requisite to comprehend its behaviour in vivo.
#'
#' This is accomplished by transforming the raw formatted data into a metabolite-specific trial.
#' Each such trial is defined as a set of linear models (n >= 30) between a timestep and number of molecules
#' for a metabolite. The coefficients of these models (slope, constant) are averaged and incorporated into a metabolite-specific
#' linear regression model. The independent variable is a representative timestep and is user-dependent.
#' The generated data is an imputed number of molecules for each metabolite, is dependent on the simulation time and
#' is defined as a "technical replicate".
#'
#' The above steps are easily repeated with additional SSA-runs (n >=30) and will generate sets of "technical replicates".
#' These can be statistically evaluated (mean, standard deviation) and will constitute the simulation time-dependent number of molecules for a particular metabolite.
#' For varying simulation times 'TemporalGSSA' will generate a metabolite-specific and simulation time-dependent trajectory.
#'
